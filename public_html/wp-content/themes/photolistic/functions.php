<?php
/**
 * photolistic functions and definitions
 *
 * Sets up the theme and provides some helper functions. Some helper functions
 * are used in the theme as custom template tags. Others are attached to action and
 * filter hooks in WordPress to change core functionality.
 *
 * The first function, photolistic_setup(), sets up the theme by registering support
 ...
 *     remove_filter( 'excerpt_length', 'photolistic_excerpt_length' );
 *     ...
 * }
 * </code>
 *
...
/** Tell WordPress to run photolistic_setup() when the 'after_setup_theme' hook is run. */
 ...
 *
 * To override photolistic_setup() in a child theme, add your own photolistic_setup to your child theme's
 * functions.php file.
 *
...
function photolistic_setup() {
	...
	if ( ! defined( 'HEADER_IMAGE' ) )
		define( 'HEADER_IMAGE', '%s/images/headers/mountains.jpg' );

	// The height and width of your custom header. You can hook into the theme's own filters to change these values.
	// Add a filter to photolistic_header_image_width and photolistic_header_image_height to change these values.
	...
	// custom headers. See photolistic_admin_header_style(), below.
 ...
 *
 * Referenced via add_custom_image_header() in photolistic_setup().
 *
 * @since Photolistic 1.0
 */
...
/**
 * Replaces "[...]" (appended to automatically generated excerpts) with an ellipsis and photolistic_continue_reading_link().
 *
 ...
 *
 * Galleries are styled by the theme in Photolistic's style.css. This is just
 ...
 *
 * @since Photolistic 1.0
 * @deprecated Deprecated in Photolistic 1.0 for WordPress 3.1
 *
...
if ( ! function_exists( 'photolistic_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * To override this walker in a child theme without modifying the comments template
 * simply create your own photolistic_comment(), and that function will be used instead.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 *
 * @since Photolistic 1.0
 */
function photolistic_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		...
		<div class="comment-author vcard">
			...
			<?php printf( __( '%s <span class="says">says:</span>', 'photolistic' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
		</div><!-- .comment-author .vcard -->
		<?php if ( $comment->comment_approved == '0' ) : ?>
			<em class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'photolistic' ); ?></em>
			<br />
		<?php endif; ?>
		...
		<div class="comment-meta commentmetadata"><a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
			<?php
				...
				printf( __( '%1$s at %2$s', 'photolistic' ), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( __( '(Edit)', 'photolistic' ), ' ' );
			?>
	...
	<li class="post pingback">
		<p><?php _e( 'Pingback:', 'photolistic' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( '(Edit)', 'photolistic' ), ' ' ); ?></p>
	<?php
			break;
	endswitch;
}
 ...
 * to remove the default style. Using Photolistic 1.0 in WordPress 3.0 will show the styles,
 * but they won't have any effect on the widget in default Photolistic styling.
 *
 * @since Photolistic 1.0
 */
...
if ( ! function_exists( 'photolistic_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 *
 * @since Photolistic 1.0
 */
function photolistic_posted_on() {
	printf( __( '<span class="%1$s">Posted on</span> %2$s <span class="meta-sep">by</span> %3$s', 'photolistic' ),
		...
		sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>',
			...
			esc_attr( sprintf( __( 'View all posts by %s', 'photolistic' ), get_the_author() ) ),
			get_the_author()
		)
	);
}
...
if ( ! function_exists( 'photolistic_posted_in' ) ) :
/**
 * Prints HTML with meta information for the current post (category, tags and permalink).
 *
 * @since Photolistic 1.0
 */
function photolistic_posted_in() {
	// Retrieves tag list of current post, separated by commas.
	$tag_list = get_the_tag_list( '', ', ' );
	if ( $tag_list ) {
		$posted_in = __( 'This entry was posted in %1$s and tagged %2$s. Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'photolistic' );
	} elseif ( is_object_in_taxonomy( get_post_type(), 'category' ) ) {
		$posted_in = __( 'This entry was posted in %1$s. Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'photolistic' );
	} else {
		$posted_in = __( 'Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'photolistic' );
	}
 ...
 * <?php echo $photolistic_option['photolistic_txt_input']; ?>
 *
 * Note: Should you notice that the variable ($photolistic_option) is empty when used in certain templates such as header.php, sidebar.php and footer.php
